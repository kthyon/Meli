...
export default async function ItemsPage({ searchParams }: { searchParams: { search: string } }) {
    ...
    const { results } = await fetch(`https://api.mercadolibre.com/sites/MLA/search?q=${encodeURIComponent(searchParams.search)}&limit=4`).then(res => res.json() as Promise<{
            results: {
                name: string;
                lastname: string;
                categories: string;
        ...
        <section>
            <div className="productoDes">
                <article className="grid gap-2.5">
                    {/* Comprobamos si hay resultados para mostrar */}
                    {results.length === 0 ? (
                        // Si no hay resultados, mostramos un mensaje indicándolo
                        <div><img src="/ups.gif" alt="404" /><p className="alert">Vaya, ese producto no está, intentalo otra vez.</p></div>
                    ) : (
                        // Si hay resultados, los mapeamos a enlaces con detalles del producto
                        results.map(item => (
